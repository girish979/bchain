...
public class Transaction {
	...
	//shows the amount relevant addresses received in the transaction. 
	...
	public boolean processTransaction() {
		...
		for(TransactionInput i : inputs) {
			i.UTXO = DonutChain.UTXOs.get(i.transactionOutputId);
		}

		//check if transaction is valid:
		if(getInputsValue() < DonutChain.minimumTransaction) {
		...
		for(TransactionOutput o : outputs) {
			DonutChain.UTXOs.put(o.id , o);
		}
			
		//remove transaction inputs from UTXO lists as spent:
		for(TransactionInput i : inputs) {
			if(i.UTXO == null) continue; //if Transaction can't be found skip it 
			DonutChain.UTXOs.remove(i.UTXO.id);
		}
